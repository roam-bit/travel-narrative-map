// 深挖 combo 的 worst_maxLatDevM：到底是评测台边界地板，还是 transform 真实甩出去？
// 复刻评测台内部的 latDev 计算逻辑，逐帧打印最大偏离帧的上下文。
...
// ---- 上报 transformCode（精简引用，与 verify_combo_review 同源）----
...
function cornerCutSmooth(rawPts) {
  const P = [];
  for (const raw of rawPts) { const p = _normPt(raw); if (p && (!P.length || _hav(P[P.length - 1], p) > 1)) P.push(p); }
...
// ---- 复刻评测台内部（buildLegs / sampleAt / latDev）----
const MIN_BRIDGE_M = 8000, LOOKAHEAD_M = 60;
...
function buildLegs(segsProcessed) {
  ...
  for (const seg of segsProcessed) { seg.forEach((cur, idx) => {
...
// 针对一章，找 latDev 最大的处理后帧位置，并报告它是否落在 bridge 邻域
function analyzeChapter(ch, transform, label) {
  ...
  // 沿 total 按米采样（不引入 ease/pace，纯几何偏离扫描）
  const STEP = 30; let worst = { dev: -1 };
  for (let m = 0; m <= pathObj.total; m += STEP) {
    const s = sampleAt(pathObj, m); if (!s) continue;
    ...
    if (best > worst.dev) worst = { dev: best, m, bridge: s.bridge, point: s.point };
  }
  // 看 worst 帧附近是否有 bridge 腿（前后 500m 内）
  let nearBridge = false; let minBridgeDist = Infinity;
  for (const l of pathObj.legs) { if (l.bridge) { const d = Math.min(Math.abs(l.start - worst.m), Math.abs(l.end - worst.m)); if (d < minBridgeDist) minBridgeDist = d; if (d < 2000) nearBridge = true; } }
  return { name: ch.name, label, worstDev: +worst.dev.toFixed(0), atM: +worst.m.toFixed(0), totalM: +pathObj.total.toFixed(0), onBridge: !!worst.bridge, nearBridge, minBridgeDistM: isFinite(minBridgeDist) ? +minBridgeDist.toFixed(0) : null, nLegs: pathObj.legs.length, nBridges: pathObj.legs.filter(l => l.bridge).length };
}
...
for (const name of ['终章 · 冰封北境与归途', '第一章 · 江南向暖', '第二章 · 一路到天涯']) {
  ...
  const c = analyzeChapter(ch, cornerCutSmooth, 'combo');
  const i = analyzeChapter(ch, cleanIdent, 'identity');
  console.log(`\n【${name}】`);
  console.log('  combo   :', JSON.stringify(c));
  console.log('  identity:', JSON.stringify(i));
  console.log(`  → combo worst比identity多 ${c.worstDev - i.worstDev}m；combo worst帧 ${c.onBridge?'在bridge上':(c.nearBridge?`距最近bridge ${c.minBridgeDistM}m`:'远离任何bridge('+c.minBridgeDistM+'m)')}`);
}
