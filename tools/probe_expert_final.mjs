...
const ex = evaluate(smoothTrackSeg,{name:'expert'});
// how many chapters exceed 800m
const over800 = ex.rows.filter(r=>r.maxLatDevM>800).map(r=>`${r.name}=${r.maxLatDevM}`);
console.log('chapters exceeding 800m maxLatDevM:', over800.length, '->', over800.join(' | '));
console.log('worst_maxLatDevM:', ex.summary.worst_maxLatDevM, '(target <= ~800)');
console.log('avg_lenPct:', ex.summary.avg_lenPct, '(target [-4,+3])  -> within range:', ex.summary.avg_lenPct>=-4 && ex.summary.avg_lenPct<=3);
console.log('smoothness vs identity(19.724): expert avg_p95AngVel=',ex.summary.avg_p95AngVel,' delta=',(ex.summary.avg_p95AngVel-19.724).toFixed(3));
